...
export default function NewTailoringRequestPage() {
    ...
    const [serviceType, setServiceType] = useState<"online" | "physical_visit">(
        "online",
    );
    ...
    useEffect(() => {
        ...
        if (user) {
            getMeasurements(user.uid)
                ...
                .catch((err) =>
                    console.log("No saved measurements found", err),
                );
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await createClothingRequest({
                ...
                target_shop_ids: shopIdParam
                    ? [parseInt(shopIdParam)]
                    : undefined,
                measurement_profile_id: savedMeasurements
                    ? savedMeasurements.measurement_id
                    : undefined,
            });
            setSubmitted(true);
            setTimeout(() => {
        ...
        try {
            await updateMeasurements(user.uid, {
                chest: measurementForm.chest
                    ? parseFloat(measurementForm.chest)
                    : undefined,
                waist: measurementForm.waist
                    ? parseFloat(measurementForm.waist)
                    : undefined,
                hip: measurementForm.hip
                    ? parseFloat(measurementForm.hip)
                    : undefined,
                shoulder: measurementForm.shoulder
                    ? parseFloat(measurementForm.shoulder)
                    : undefined,
                sleeve: measurementForm.sleeve
                    ? parseFloat(measurementForm.sleeve)
                    : undefined,
                neck: measurementForm.neck
                    ? parseFloat(measurementForm.neck)
                    : undefined,
                inseam: measurementForm.inseam
                    ? parseFloat(measurementForm.inseam)
                    : undefined,
                length: measurementForm.length
                    ? parseFloat(measurementForm.length)
                    : undefined,
        ...
        <main className="max-w-3xl w-full mx-auto px-4 sm:px-8 py-8 flex-1 space-y-8 bg-warm-beige min-h-screen text-earth-text selection:bg-accent selection:text-cream-bg">
            ...
            <div className="bg-cream-bg border border-accent/20 rounded-2xl p-6 sm:p-8 shadow-md">
                ...
                <p className="text-xs sm:text-sm text-earth-text/70 mt-2 leading-relaxed font-medium">
                    Specify your bespoke garment requirements to connect with
                    top Sri Lankan master tailors.
                </p>
            </div>
            ...
            {submitted ? (
                <div className="bg-cream-bg border border-accent/40 rounded-2xl p-8 text-center space-y-4 shadow-md">
                    ...
                    <h2 className="text-xl font-bold text-earth-text font-heading">
                        Request Dispatched!
                    </h2>
                    <p className="text-xs text-earth-text/70 font-medium">
                        Local ateliers{city ? ` in ${city}` : ""} are reviewing
                        your specification. Redirecting to your dashboard...
                    </p>
                </div>
            ) : (
                <form
                    onSubmit={handleSubmit}
                    className="bg-cream-bg border border-accent/20 rounded-2xl p-6 sm:p-10 shadow-md space-y-6"
                >
                    {/* Error Banner */}
                    {error && (
                    ...
                    <div>
                        ...
                        >
                            <option value="TWO-PIECE SUIT">
                                TWO-PIECE SUIT (JACKET &amp; TROUSERS)
                            </option>
                            <option value="THREE-PIECE TUXEDO">
                                THREE-PIECE TUXEDO
                            </option>
                            <option value="OVERCOAT / TRENCH">
                                OVERCOAT / TRENCH
                            </option>
                            <option value="BESPOKE SHIRT">BESPOKE SHIRT</option>
                            <option value="ALTERATION &amp; REPAIR">
                                ALTERATION &amp; REPAIR
                            </option>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <button
                                ...
                                className={`py-3.5 rounded-xl text-xs font-bold uppercase tracking-wider border transition-all flex flex-col items-center gap-1 ${
                                    fabricChoice === "shop_provides"
                                        ? "bg-accent text-cream-bg border-accent shadow-sm"
                                        : "bg-warm-beige/60 text-earth-text/80 border-accent/30 hover:border-accent"
                                }`}
                            >
                            ...
                            <button
                                type="button"
                                onClick={() =>
                                    setFabricChoice("client_provided")
                                }
                                className={`py-3.5 rounded-xl text-xs font-bold uppercase tracking-wider border transition-all flex flex-col items-center gap-1 ${
                                    fabricChoice === "client_provided"
                                        ? "bg-accent text-cream-bg border-accent shadow-sm"
                                        : "bg-warm-beige/60 text-earth-text/80 border-accent/30 hover:border-accent"
                                }`}
                            >
                    ...
                    <div className="bg-warm-beige/80 p-4 rounded-xl border border-accent/20 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-sm font-bold text-earth-text">
                                    Measurement Profile
                                </p>
                                <p className="text-xs text-earth-text/70 mt-0.5 font-medium">
                                    {savedMeasurements
                                        ? "Your saved bespoke measurements will be attached securely to this request."
                                        : "No saved measurements found. Tailors will ask for them later."}
                                </p>
                            </div>
                        </div>
                        <button
                    ...
                    <div className="p-5 bg-warm-beige/50 rounded-xl space-y-5 border border-accent/20">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full text-xs text-earth-text/70 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-cream-bg file:text-earth-text hover:file:bg-accent hover:file:text-cream-bg transition-all cursor-pointer"
                            />
                        </div>
                            ...
                            <div className="flex flex-wrap items-center gap-4">
                                {isRecording ? (
                                    <button
                                        ...
                                        className="px-5 py-2.5 bg-rose-500/10 text-rose-600 border border-rose-500/30 rounded-xl text-xs font-bold animate-pulse hover:bg-rose-500/20 transition-all"
                                    >
                                        Stop Recording
                                    </button>
                                ) : (
                                    <button
                                        ...
                                        className="px-5 py-2.5 bg-cream-bg text-earth-text border border-accent/30 rounded-xl text-xs font-bold hover:bg-accent hover:text-cream-bg transition-all"
                                    >
                                        Record Audio
                                    </button>
                                )}
                        ...
                        <div className="bg-cream-bg rounded-2xl border border-accent/20 w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
                            ...
                            <div className="p-6 space-y-6">
                                <p className="text-xs text-earth-text/70">
                                    Save these once, and automatically apply
                                    them to all future bespoke requests. All
                                    values in inches.
                                </p>
                                <form
                                ...
                                >
                                    <div className="grid grid-cols-2 gap-x-4 gap-y-6">
                                        <div>
                                            <label className="text-[10px] font-mono tracking-widest text-accent uppercase block mb-1 font-semibold">
                                                Chest
                                            </label>
                                            <input
                                                type="number"
                                                ...
                                                className="w-full px-4 py-3 bg-white/90 border border-accent/20 focus:border-accent rounded-xl text-sm text-earth-text focus:outline-none transition-all text-center font-mono"
                                            />
                                        </div>
                                        <div>
                                            <label className="text-[10px] font-mono tracking-widest text-accent uppercase block mb-1 font-semibold">
                                        ...
                                        <div className="col-span-2">
                                            ...
                                            <textarea
                                                ...
                                                className="w-full px-4 py-3 bg-white/90 border border-accent/20 focus:border-accent rounded-xl text-sm text-earth-text focus:outline-none transition-all resize-none font-mono"
                                            />
                                        </div>
                                    </div>
