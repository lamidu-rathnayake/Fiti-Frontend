...
export default function TailorHomePage() {
    ...
    const SkeletonCard = () => <div className="h-40 bg-cream-bg border border-accent/20 rounded-2xl animate-pulse" />;
    ...
    const renderInquiryCard = (view: RequestView) => {
        ...
        return (
            ...
            >
                <div className={`absolute top-0 left-0 w-1.5 h-full ${isDirect ? "bg-amber-700" : "bg-accent"}`} />
                <div className="p-5 pl-6 flex-1">
                    <div className="flex justify-between items-start mb-2">
                        <div>
                            <span className={`text-[9px] font-mono font-bold uppercase tracking-widest px-2.5 py-0.5 rounded-full border mb-1.5 inline-block ${isDirect ? "text-amber-900 bg-amber-700/10 border-amber-700/30" : "text-accent bg-accent/10 border-accent/20"}`}>
                                {isDirect ? "Direct Inquiry" : "Bidding Request"}
                            </span>
                            <h4 className="text-base font-extrabold text-earth-text font-heading leading-tight">{req.clothing_category || "Custom Garment"}</h4>
                        </div>
                        <span className="text-xs font-mono font-bold text-accent">#{req.request_id}</span>
                    </div>
                    ...
                    {req.description && <p className="text-xs text-earth-text/60 italic line-clamp-2 bg-warm-beige p-3 rounded-xl border border-accent/15">"{req.description}"</p>}
                </div>
                
                {canSubmit ? (
                    <div className="p-4 bg-warm-beige border-t border-accent/20 space-y-2.5" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="p-3 bg-warm-beige border-t border-accent/20 text-center" onClick={(e) => e.stopPropagation()}>
                        <span className="text-xs font-bold text-earth-text/60">Shop request not initialized</span>
                    </div>
                )}
            </div>
        );
        ...
        return (
            ...
            >
                ...
                <div className="p-5 pl-6">
                    <div className="flex justify-between items-start mb-2">
                        <h4 className="text-base font-extrabold text-earth-text font-heading leading-tight">{req.clothing_category || "Custom Garment"}</h4>
                        <span className="text-[10px] font-mono uppercase font-bold text-accent bg-accent/10 px-2.5 py-0.5 rounded-full border border-accent/20">Quoted</span>
                    </div>
                    <div className="bg-warm-beige border border-accent/20 rounded-xl p-3 flex justify-between items-center mt-3">
                        <span className="text-[10px] font-mono text-earth-text/60 uppercase font-bold">Your Offer</span>
                        <span className="text-sm font-black text-accent font-mono">LKR {Number(myShopRequest?.offered_price || 0).toLocaleString()}</span>
                    </div>
                    <p className="text-[10px] text-earth-text/60 mt-3 text-center uppercase tracking-wider font-bold">Awaiting Client Approval</p>
                </div>
            </div>
        );
    };
    ...
    const renderOrderCard = (ord: Order, isActive: boolean) => {
        return (
            ...
            >
                <div className={`absolute top-0 left-0 w-1.5 h-full ${isActive ? "bg-emerald-600" : "bg-earth-text/40"}`} />
                <div className="p-5 pl-6">
                    <div className="flex justify-between items-start mb-1">
                        <h4 className="text-base font-extrabold text-earth-text font-heading leading-tight">Order #{ord.order_id}</h4>
                        <span className="text-xs font-mono font-bold text-earth-text/50">Ref #{ord.shop_request_id}</span>
                    </div>
                    <p className="text-sm font-black text-accent font-mono mb-3">LKR {Number(ord.accepted_price).toLocaleString()}</p>
                    
                    {isActive ? (
                        <div className="bg-emerald-600/10 rounded-xl p-2.5 text-center border border-emerald-600/20">
                            <span className="text-[10px] font-mono text-emerald-800 uppercase font-bold tracking-wider">In Progress</span>
                        </div>
                    ) : (
                        <div className="bg-earth-text/5 rounded-xl p-2.5 text-center border border-earth-text/10">
                            <span className="text-[10px] font-mono text-earth-text/60 uppercase font-bold tracking-wider">Finished</span>
                        </div>
                    )}
                </div>
            </div>
    ...
    return (
        <div className="min-h-screen bg-warm-beige text-earth-text selection:bg-accent selection:text-cream-bg font-sans flex flex-col">
            <FullPageLock
                isSubmitting={submittingBidFor != null || completingOrderId != null}
            ...
            <header className="h-16 shrink-0 border-b border-accent/20 bg-cream-bg flex items-center px-6 lg:px-12 justify-between shadow-sm">
                <div className="flex items-center gap-8">
                    <span className="text-earth-text font-serif font-black text-xl tracking-tight">Atelier Mode</span>
                ...
                <div className="relative">
                    <button onClick={() => setIsShopDropdownOpen(!isShopDropdownOpen)} className="flex items-center gap-2 bg-warm-beige border border-accent/30 hover:border-accent px-4 py-2 rounded-full text-xs font-bold text-earth-text transition-all shadow-sm">
                        <span className="truncate max-w-[140px]">{shopDisplayName}</span>
                        <svg className={`w-3.5 h-3.5 text-accent transition-transform ${isShopDropdownOpen ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                    </button>
                    {isShopDropdownOpen && (
                        <div className="absolute top-full right-0 mt-2 w-52 bg-cream-bg border border-accent/20 rounded-2xl shadow-xl overflow-hidden z-20 py-1.5">
                            {tailorShops.map(shop => (
                                <button key={shop.shop_id} onClick={() => handleSelectShop(shop)} className="w-full text-left px-4 py-2.5 text-xs font-bold hover:bg-warm-beige transition-colors text-earth-text/80 hover:text-earth-text">
                                    {shop.shop_name}
                                </button>
                            ))}
                            <div className="h-px bg-accent/15 my-1.5"/>
                            <Link href="/tailor/add-shop" className="w-full text-left px-4 py-2 text-xs font-bold text-accent hover:bg-warm-beige transition-colors flex items-center gap-2">
                                + Add New Shop
                            </Link>
                        </div>
                    )}
            ...
            <main className="flex-1 overflow-hidden flex flex-col bg-warm-beige">
                    ...
                    <div className="p-6 lg:p-12 overflow-y-auto custom-scrollbar h-full">
                        <div className="max-w-4xl mx-auto space-y-8">
                            <div>
                                ...
                                <h1 className="text-3xl font-extrabold text-earth-text font-heading">Dashboard Overview</h1>
                                <p className="text-earth-text/70 text-xs mt-1 font-medium">High-level statistics for {shopDisplayName}</p>
                            </div>

                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {[
                                    { label: "New Inquiries", value: newInquiries.length, color: "text-earth-text" },
                                    { label: "Pending Quotes", value: pendingClient.length, color: "text-accent font-mono" },
                                    { label: "Active Orders", value: activeWorkshop.length, color: "text-emerald-800" },
                                    { label: "Completed", value: completedHistory.length, color: "text-earth-text/70" },
                                ].map((stat, i) => (
                                    <div key={i} className="bg-cream-bg border border-accent/20 p-5 rounded-2xl shadow-sm">
                                        <p className="text-[10px] font-mono font-bold uppercase tracking-widest text-earth-text/60 mb-2">{stat.label}</p>
                            ...
                            <div className="space-y-6 pt-4">
                                <div>
                                    <h2 className="text-base font-extrabold text-earth-text font-heading mb-4">New Inquiries ({newInquiries.length})</h2>
                                    {isLoading ? (
                                ...
                                <div>
                                    <h2 className="text-base font-extrabold text-earth-text font-heading mb-4">Pending Client Approval ({pendingClient.length})</h2>
                                    {pendingClient.length === 0 ? (
                                        <div className="bg-cream-bg border border-accent/20 rounded-2xl p-8 text-center text-xs text-earth-text/60 shadow-sm font-medium">
                                            No quotations currently awaiting client response.
                                        </div>
                                    ) : (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <h2 className="text-base font-extrabold text-earth-text font-heading mb-4">Active Orders ({activeWorkshop.length})</h2>
                    ...
                    <div className="p-6 lg:p-12 overflow-y-auto custom-scrollbar h-full">
                        <div className="max-w-4xl mx-auto space-y-8">
                            <div>
                                ...
                                <h1 className="text-3xl font-extrabold text-earth-text font-heading">Earnings & Revenue</h1>
                                <p className="text-earth-text/70 text-xs mt-1 font-medium">Financial overview for {shopDisplayName}</p>
                            </div>

                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="bg-cream-bg border border-accent/20 p-6 rounded-2xl shadow-sm">
                                    <p className="text-[10px] font-mono font-bold uppercase tracking-widest text-earth-text/60 mb-2">Total Earned</p>
                                    <p className="text-4xl font-black text-emerald-800 font-mono">LKR {totalRevenue.toLocaleString()}</p>
                                </div>
                                <div className="bg-cream-bg border border-accent/20 p-6 rounded-2xl shadow-sm">
                                    <p className="text-[10px] font-mono font-bold uppercase tracking-widest text-earth-text/60 mb-2">Pipeline Value (In Progress)</p>
                                    <p className="text-4xl font-black text-accent font-mono">LKR {pipelineValue.toLocaleString()}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-6 lg:p-12 overflow-y-auto custom-scrollbar h-full">
                        <div className="max-w-2xl mx-auto space-y-8">
                            <div>
                                ...
                                <h1 className="text-3xl font-extrabold text-earth-text font-heading">Tailor Settings</h1>
                                <p className="text-earth-text/70 text-xs mt-1 font-medium">Manage your professional profile and status.</p>
                            </div>

                            <form className="bg-cream-bg border border-accent/20 rounded-2xl p-6 sm:p-8 space-y-5 shadow-sm">
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-mono font-bold uppercase tracking-widest text-earth-text/70">Firebase Email</label>
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-mono font-bold uppercase tracking-widest text-earth-text/70">Verification Status</label>
                ...
                {selectedRequestView && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-earth-text/40 backdrop-blur-sm" onClick={() => setSelectedRequestView(null)}>
                        <div className="bg-cream-bg border border-accent/30 w-full max-w-4xl max-h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden text-earth-text" onClick={e => e.stopPropagation()}>
                            {/* Header */}
                            <div className="px-6 py-4 border-b border-accent/20 flex justify-between items-center bg-warm-beige shrink-0">
                                <div className="flex items-center gap-3">
                                    <span className="text-accent font-mono text-xs font-bold">#{selectedRequestView.req.request_id}</span>
                                    <h2 className="text-lg font-extrabold text-earth-text font-heading">{selectedRequestView.req.clothing_category || "Custom Garment Request"}</h2>
                                </div>
                                <button onClick={() => setSelectedRequestView(null)} className="text-earth-text/50 hover:text-earth-text transition-colors p-1">
                            ...
                            <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
                                ...
                                <div className="flex-1 p-6 overflow-y-auto custom-scrollbar border-b md:border-b-0 md:border-r border-accent/20 space-y-6">
                                    <div className="space-y-2">
                                        <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest">Description</h3>
                                    ...
                                    {selectedRequestView.req.design_images && selectedRequestView.req.design_images.length > 0 && (
                                        <div className="mb-6">
                                            <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-3">Reference Images</h3>
                                            <div className="flex gap-3 overflow-x-auto pb-2 snap-x">
                                                {selectedRequestView.req.design_images.map((img, i) => (
                                                    <img key={i} src={img.image_url} alt={`Ref ${i+1}`} className="h-32 w-32 object-cover rounded-xl border border-accent/20 snap-start shadow-sm" />
                                                ))}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="bg-warm-beige p-4 rounded-xl border border-accent/20 flex flex-col justify-center">
                                            <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-1">Target Budget</h3>
                                        ...
                                        <div className="bg-warm-beige p-4 rounded-xl border border-accent/20 flex flex-col justify-center">
                                            <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-1">Target Date</h3>
                                        ...
                                        <div className="bg-warm-beige p-4 rounded-xl border border-accent/20 flex flex-col justify-center">
                                            <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-1">Fabric Status</h3>
                                        ...
                                        <div className="bg-warm-beige p-4 rounded-xl border border-accent/20 flex flex-col justify-center">
                                            <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-1">Gender Fit</h3>
                                            <p className="text-xs font-bold text-earth-text capitalize">
                                    ...
                                    {selectedRequestView.req.client && (
                                        <div className="space-y-2 pt-4 border-t border-accent/20">
                                            <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest">Client Details</h3>
                                            <div className="bg-warm-beige p-4 rounded-xl border border-accent/20 grid grid-cols-2 gap-4">
                                                <div>
                                                    <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-1">Name</h3>
                                                    <p className="text-xs font-bold text-earth-text">{selectedRequestView.req.client.display_name || "Unknown Client"}</p>
                                                </div>
                                                {selectedRequestView.req.client.phone && (
                                                    <div>
                                                        <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-1">Contact</h3>
                                                        <p className="text-xs font-bold text-earth-text">{selectedRequestView.req.client.phone}</p>
                                                    </div>
                                                )}
                                                {selectedRequestView.req.client.city && (
                                                    <div className="col-span-2">
                                                        <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-1">Location</h3>
                                                        <p className="text-xs font-bold text-earth-text">{selectedRequestView.req.client.city}</p>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-2 pt-4 border-t border-accent/20">
                                            ...
                                            <div className="bg-warm-beige p-4 rounded-xl border border-accent/20 grid grid-cols-3 gap-y-4 gap-x-2">
                                                {['chest', 'waist', 'shoulder', 'sleeve', 'neck', 'hip', 'inseam', 'length'].map((part) => {
                                                    ...
                                                    return (
                                                        <div key={part}>
                                                            <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-1">{part}</h3>
                                                            <p className="text-xs font-bold text-earth-text font-mono">{val} cm</p>
                                                        </div>
                                                    );
                                                })}
                                                {selectedRequestView.req.measurement.notes && (
                                                    <div className="col-span-3 mt-2">
                                                        <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-1">Notes</h3>
                                                        <p className="text-xs text-earth-text/80 italic">"{selectedRequestView.req.measurement.notes}"</p>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                ...
                                <div className="w-full md:w-80 p-6 bg-warm-beige overflow-y-auto custom-scrollbar flex flex-col space-y-6 shrink-0">
                                    <div className="space-y-4">
                                        <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest">Status & Actions</h3>
                                        
                                        {/* Status indicator */}
                                        <div className="flex items-center gap-2">
                                            <div className={`w-2.5 h-2.5 rounded-full ${selectedRequestView.myShopRequest?.status === 'quoted' ? 'bg-accent' : 'bg-amber-700'}`} />
                                        ...
                                        {selectedRequestView.myShopRequest?.status === 'quoted' ? (
                                            <div className="bg-cream-bg border border-accent/20 rounded-xl p-4 shadow-sm">
                                                <h4 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-2">Your Submitted Offer</h4>
                                                <p className="text-xl font-black text-accent font-mono">LKR {Number(selectedRequestView.myShopRequest.offered_price || 0).toLocaleString()}</p>
                                                <p className="text-xs text-earth-text/70 mt-2">Waiting for client to accept or reject.</p>
                                            </div>
                                        ) : (
                                            <div className="bg-cream-bg border border-accent/20 rounded-2xl p-4 space-y-4 shadow-sm">
                                                <div className="space-y-1">
                                                    <label className="text-[10px] font-mono font-bold text-earth-text/70 uppercase tracking-widest">Quote Amount (LKR)</label>
                                                    <input 
                                                        type="number" 
                                                ...
                                                <div className="space-y-1">
                                                    <label className="text-[10px] font-mono font-bold text-earth-text/70 uppercase tracking-widest">Message to Client</label>
                                                    <textarea 
                ...
                {/* MODAL: Order Details */}
                {selectedOrderView && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-earth-text/40 backdrop-blur-sm" onClick={() => setSelectedOrderView(null)}>
                        <div className="bg-cream-bg border border-accent/30 w-full max-w-2xl max-h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden text-earth-text" onClick={e => e.stopPropagation()}>
                            <div className="px-6 py-4 border-b border-accent/20 flex justify-between items-center bg-warm-beige shrink-0">
                                <div className="flex items-center gap-3">
                                    <span className="text-accent font-mono text-xs font-bold">#{selectedOrderView.order.order_id}</span>
                                    <h2 className="text-lg font-extrabold text-earth-text font-heading">Order Details</h2>
                                </div>
                                <button onClick={() => setSelectedOrderView(null)} className="text-earth-text/50 hover:text-earth-text transition-colors p-1">
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                                </button>
                            </div>
                            ...
                            <div className="p-6 overflow-y-auto custom-scrollbar space-y-6">
                                <div className="flex items-center justify-between p-4 bg-warm-beige border border-accent/20 rounded-2xl">
                                    <div>
                                        <p className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-1">Accepted Price</p>
                                        <p className="text-2xl font-black text-accent font-mono">LKR {Number(selectedOrderView.order.accepted_price).toLocaleString()}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-1">Status</p>
                                        <div className="inline-block px-3 py-1 rounded-full border bg-emerald-600/10 border-emerald-600/20">
                                            <p className="text-xs font-bold text-emerald-800 capitalize">{selectedOrderView.order.order_status.replace("_", " ")}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-warm-beige p-4 rounded-xl border border-accent/20 flex flex-col justify-center">
                                        <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-1">Started Date</h3>
                                        <p className="text-xs font-bold text-earth-text">
                                            {selectedOrderView.order.started_date ? new Date(selectedOrderView.order.started_date).toLocaleDateString() : "N/A"}
                                        </p>
                                    </div>
                                    <div className="bg-warm-beige p-4 rounded-xl border border-accent/20 flex flex-col justify-center">
                                        <h3 className="text-[10px] font-mono font-bold text-earth-text/60 uppercase tracking-widest mb-1">Ref Request ID</h3>
                                        <p className="text-xs font-mono font-bold text-earth-text">#{selectedOrderView.order.shop_request_id}</p>
                                    </div>
                                </div>
                                ...
                                {selectedOrderView.isActive && (
                                    <div className="pt-4 border-t border-accent/20">
                                        ...
                                        <p className="text-center text-xs text-earth-text/60 mt-3 font-medium">This action will notify the client and finalize the order.</p>
                                    </div>
                                )}
                            </div>
                        </div>
