...
export default function TailorOrdersPage() {
    ...
    const [activeTab, setActiveTab] = useState<
        "requests" | "quoted" | "ongoing" | "complete"
    >("requests");
    ...
    const mapRequestToOrderDetail = (
        ...
        if (cat.includes("overcoat") || cat.includes("coat"))
        ...
        const clientName = req.client?.display_name
            ...
            : req.client_id
              ? req.client_id.length > 15
                  ? req.client_id.substring(0, 10) + "..."
                  : req.client_id
              : "Client";
        ...
        return {
            ...
            targetDate: req.target_date
                ? new Date(req.target_date).toLocaleDateString("en-US", {
                      month: "2-digit",
                      day: "2-digit",
                      year: "numeric",
                  })
            ...
            quotedPrice: quotedPrice
                ? `LKR ${Number(quotedPrice).toLocaleString()}`
                : undefined,
            garmentType,
            fitPreference: isFemale
                ? "Women's Fit"
                : req.gender === "unisex"
                  ? "Unisex"
                  : req.gender === "male"
                    ...
                    : "Not specified",
            materialSourcing:
                req.fabric_status === "client_provided"
                    ? "Providing Fabric"
                    : req.fabric_status
                      ? "Need Sourcing"
                      : "Not specified",
            measurements: {
                chest: req.measurement?.chest
                    ? String(req.measurement.chest)
                    : "Not specified",
                waist: req.measurement?.waist
                    ? String(req.measurement.waist)
                    : "Not specified",
                sleeve: req.measurement?.sleeve
                    ? String(req.measurement.sleeve)
                    : "Not specified",
                neck: req.measurement?.neck
                    ? String(req.measurement.neck)
                    : "Not specified",
                shoulder: req.measurement?.shoulder
                    ? String(req.measurement.shoulder)
                    : "Not specified",
    ...
    const mapOrderToOrderDetail = (ord: Order): OrderDetail => {
        ...
        const statusLabel =
            orderStatus === "in_progress"
                ? "IN FITTING"
                : (orderStatus || "pending").replace(/_/g, " ").toUpperCase();
        ...
        const clientName = req?.client?.display_name
            ...
            : req?.client_id
              ? req.client_id.length > 15
                  ? req.client_id.substring(0, 10) + "..."
                  : req.client_id
              : `Client #${ord.shop_request_id}`;

        const isFemale = req?.gender === "female";
        const garmentTypeStr = req?.clothing_category
            ? req.clothing_category.charAt(0).toUpperCase() +
              req.clothing_category.slice(1)
            : "Suit";

        let garmentType: "Suit" | "Shirt" | "Overcoat" | "Trousers" = "Suit";
        if (
            garmentTypeStr.includes("Shirt") ||
            garmentTypeStr.includes("shirt")
        )
            garmentType = "Shirt";
        else if (
            garmentTypeStr.includes("Coat") ||
            garmentTypeStr.includes("coat")
        )
            garmentType = "Overcoat";
        else if (
            garmentTypeStr.includes("Trouser") ||
            garmentTypeStr.includes("Pant") ||
            garmentTypeStr.includes("trouser") ||
            garmentTypeStr.includes("pant")
        )
            garmentType = "Trousers";
        ...
        return {
            ...
            date: ord.created_at
                ...
                : "—",
            targetDate: ord.completed_date
                ? new Date(ord.completed_date).toLocaleDateString("en-US", {
                      month: "2-digit",
                      day: "2-digit",
                      year: "numeric",
                  })
            ...
            budget: req?.target_budget
                ? `LKR ${Number(req.target_budget).toLocaleString()}`
                : "Custom Quote",
            quotedPrice: ord.accepted_price
            ...
            designNotes:
                req?.description ||
                "No additional design notes on file for this order.",
            images:
                req?.design_images && req.design_images.length > 0
                    ? req.design_images.map((img) => img.image_url)
                    : [],
    ...
    const fetchTailorData = useCallback(async () => {
        ...
        try {
            ...
            const shopReqsPromise =
                shopId !== null
                    ? listShopRequests(shopId)
                    : Promise.resolve([]);
            const shopOrdersPromise =
                shopId !== null ? listShopOrders(shopId) : Promise.resolve([]);
            ...
            Array.from(allReqsMap.values()).forEach((req) => {
                const mySr = shopId
                    ? req.shop_requests?.find((sr) => sr.shop_id === shopId)
                    : null;
                const isBidding = req.request_type === "bidding";

                if (
                    mySr &&
                    (mySr.status === "quoted" || mySr.status === "rejected")
                ) {
                    const mapped = mapRequestToOrderDetail(req);
                    mapped.shopRequestId = mySr.shop_request_id;
                    mapped.quotedPrice = mySr.offered_price
                        ? `LKR ${Number(mySr.offered_price).toLocaleString()}`
                        : undefined;
                    mapped.status =
                        mySr.status === "rejected" ? "REJECTED" : "QUOTED";
                    quoted.push(mapped);
                } else if (
                    (mySr && mySr.status === "pending") ||
                    (!mySr && isBidding)
                ) {
                    const mapped = mapRequestToOrderDetail(req);
                    if (mySr) {
                        // Keep rawId as the clothing request ID for cancellation.
                    }
                    newInqs.push(mapped);
                }
            });
            ...
            setIsDbConnected(
                openReqs.status === "fulfilled" ||
                    shopReqs.status === "fulfilled" ||
                    shopOrders.status === "fulfilled",
            );
        } catch (err) {
    ...
    const handleIssueQuotation = async (req: OrderDetail) => {
        ...
        try {
            await submitBid({
                shop_request_id: req.shopRequestId ?? req.rawId,
    ...
    const renderOrderAtelierDetails = (order: OrderDetail) => {
        const bidsForThisShop =
            order.bids?.filter(
                (b) =>
                    b.shop_request_id === (order.shopRequestId ?? order.rawId),
            ) || [];
        ...
        return (
            <div className="space-y-4 pt-4 border-t border-accent/20 text-left">
                    ...
                    <div className="bg-warm-beige rounded-2xl p-4 border border-accent/30 space-y-4 mt-4 relative overflow-hidden">
                        <div>
                            <span className="text-[10px] font-mono font-bold uppercase tracking-[0.25em] text-accent block mb-0.5">
                                {order.status === "QUOTED"
                                    ? "Counter-Bid Options"
                                    : order.status === "REJECTED"
                                      ? "Client Rejected - New Bid?"
                                      : "Issue Quotation"}
                            </span>
                            <p className="text-[11px] text-earth-text/70 font-medium">
                                {order.status === "REJECTED"
                                    ? "The client declined your previous quotation. Submit a better offer to win this commission."
                                    : "Submit your bid and message to the client directly."}
                            </p>
                        </div>

                        <div className="space-y-3">
                            <div>
                                <label className="text-[9px] font-mono font-bold tracking-[0.2em] uppercase text-earth-text/60 block mb-1.5">
                                    OFFERED PRICE (LKR)
                                </label>
                                <input
                                    type="number"
                        ...
                        {bidsForThisShop.length > 0 && (
                            <div className="mt-6 pt-4 border-t border-accent/20">
                                <h4 className="text-[9px] font-mono font-bold tracking-[0.2em] uppercase text-earth-text/60 mb-2">
                                    Bid History
                                </h4>
                                <div className="space-y-2">
                                    {[...bidsForThisShop]
                                        ...
                                        .map((b, i) => (
                                            <div
                                                key={b.bid_id || i}
                                                className="bg-cream-bg border border-accent/10 rounded-lg p-2.5 text-xs flex flex-col gap-1 shadow-sm"
                                            >
                                                ...
                                                {b.message && (
                                                    <span className="text-earth-text/70 italic block">
                                                        "{b.message}"
                                                    </span>
                                                )}
                                            </div>
                ...
                <div>
                    <span className="text-[10px] font-mono font-bold uppercase tracking-[0.25em] text-earth-text/60 block mb-0.5">
                        Request Details
                    </span>
                    <p className="text-[11px] text-earth-text/70 font-medium">
                        Live specifics for this bespoke commission.
                    </p>
                </div>
                ...
                <div className="bg-warm-beige rounded-2xl p-4 border border-accent/20 space-y-3">
                    ...
                    <div>
                        ...
                        <div className="w-full bg-cream-bg border-b border-accent/30 px-3.5 py-2.5 rounded-lg text-xs text-earth-text font-medium flex flex-col gap-1">
                            <span className="flex items-center gap-1.5">
                                ...
                                {order.clientCity || "Not specified"}
                            </span>
                            <span className="flex items-center gap-1.5">
                                <svg
                                    className="w-3.5 h-3.5 text-accent"
                    ...
                    {order.quotedPrice && (
                        <div>
                            ...
                            <div className="w-full bg-cream-bg border-b-2 border-accent px-3.5 py-2.5 rounded-lg text-xs text-accent font-black font-mono">
                                {order.quotedPrice}
                            </div>
                        </div>
                    )}
                ...
                <div className="bg-warm-beige rounded-2xl p-4 border border-accent/20 space-y-4">
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {["Suit", "Shirt", "Overcoat", "Trousers"].map(
                                (type) => {
                                    const isSelected =
                                        order.garmentType === type;
                                    return (
                                        <span
                                            key={type}
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {["Men's Fit", "Women's Fit", "Unisex"].map(
                                (fit) => {
                                    const isSelected =
                                        order.fitPreference === fit;
                                    return (
                                        <span
                                            key={fit}
                                            className={`px-4 py-1.5 rounded-full text-xs font-mono font-bold transition-all ${
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            >
                                <svg
                                    className="w-5 h-5 mb-1 opacity-80"
                                    fill="none"
                                    stroke="currentColor"
                                    viewBox="0 0 24 24"
                                >
                ...
                <div className="bg-warm-beige rounded-2xl p-4 border border-accent/20 space-y-4">
                    <div className="flex justify-between items-center">
                        <label className="text-[9px] font-mono font-bold tracking-[0.2em] uppercase text-earth-text/60">
                            KEY MEASUREMENTS
                        </label>
                    ...
                    <div className="grid grid-cols-2 gap-3 font-mono">
                        <div>
                            <span className="text-[9px] font-mono text-earth-text/60 font-bold block mb-1">
                                CHEST (CM)
                            </span>
                    ...
                    {order.measurements.notes && (
                        <div className="mt-4 p-3 bg-cream-bg border border-accent/20 rounded-xl">
                            ...
                            <p className="text-xs text-earth-text/80 leading-relaxed font-sans">
                                {order.measurements.notes}
                            </p>
                        </div>
                    )}
                    <p className="text-[10px] text-earth-text/50 italic mt-2">
                        Measurements not on file yet appear as &quot;Not
                        specified&quot;.
                    </p>
                    ...
                    <div>
                        ...
                        {order.images && order.images.length > 0 ? (
                            ...
                            <div className="border border-dashed border-accent/40 bg-cream-bg/50 rounded-2xl p-6 flex flex-col items-center justify-center text-center space-y-2">
                                ...
                                <div>
                                    <span className="text-xs font-bold text-earth-text block">
                                        No Reference Image
                                    </span>
                                    <span className="text-[10px] text-earth-text/60">
                                        No sketches attached to this order.
                                    </span>
                                </div>
                            </div>
                        )}
    ...
    return (
        <div className="min-h-screen bg-warm-beige text-earth-text flex flex-col justify-between selection:bg-accent selection:text-cream-bg font-sans">
            ...
            <main className="max-w-7xl w-full mx-auto px-4 sm:px-8 py-12 flex-1 space-y-8">
                <div className="flex items-center justify-between gap-4 flex-wrap">
                    ...
                    <div className="flex items-center gap-2 px-3 py-1.5 bg-cream-bg rounded-full border border-accent/20 text-[10px] text-earth-text/70 shadow-sm">
                        <span
                            className={`w-2 h-2 rounded-full ${isDbConnected ? "bg-emerald-600 shadow-sm" : "bg-amber-600"}`}
                        ></span>
                ...
                {activeTab === "requests" && (
                    <div className="space-y-4 p-2 rounded-[28px]">
                        <h2 className="text-[11px] font-mono font-bold tracking-[0.2em] uppercase text-accent">
                            New Inquiries ({requests.length})
                        </h2>
                        ...
                        {requests.length === 0 ? (
                            ...
                            <div className="space-y-4">
                                {requests.map((req) => (
                                    <div
                                        key={req.id}
                                        className="bg-cream-bg border border-accent/20 hover:border-accent/50 rounded-2xl p-5 shadow-sm hover:shadow-md relative overflow-hidden transition-all"
                                    >
                                        ...
                                        >
                                                ...
                                                {req.images &&
                                                    req.images[0] && (
                                                        <img
                                                            ...
                                                            className="w-12 h-12 rounded-xl object-cover border border-accent/30 shrink-0"
                                                        />
                                                    )}
                                                <div>
                                            ...
                                            <div className="text-right">
                                                <span className="text-xs font-mono font-bold text-accent bg-warm-beige px-3 py-1 rounded-xl border border-accent/20 inline-block">
                                                    {req.budget}
                                                </span>
                ...
                {activeTab === "quoted" && (
                    <div className="space-y-4 p-2 rounded-[28px]">
                        <h2 className="text-[11px] font-mono font-bold tracking-[0.2em] uppercase text-earth-text/60">
                            Pending Quotations ({pendingQuotes.length})
                        </h2>
                        ...
                        {pendingQuotes.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {pendingQuotes.map((ord) => (
                                    <div
                                        key={ord.id}
                                        className="bg-cream-bg border border-accent/20 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all"
                                    >
                                        <div
                                            onClick={() =>
                                        ...
                                        >
                                            ...
                                            <div className="text-right flex flex-col items-end gap-1">
                                                <span className="text-[10px] font-mono font-bold text-earth-text/60">
                                                    Requested: {ord.budget}
                                                </span>
                                                {ord.quotedPrice && (
                                                ...
                                                {ord.status === "REJECTED" ? (
                                                    <span className="text-[9px] font-mono font-bold text-red-700 uppercase tracking-widest bg-red-600/10 border border-red-600/20 px-2.5 py-1 rounded-full">
                                                        Rejected by Client
                                                    </span>
                                                ) : (
                                                    <span className="text-[9px] font-mono font-bold text-accent uppercase tracking-widest bg-accent/10 border border-accent/20 px-2.5 py-1 rounded-full">
                                                        {ord.status}
                                                    </span>
                                                )}
                                                <span className="text-[10px] font-bold text-earth-text/50 group-hover:text-accent transition-colors mt-1 block">
                        ...
                        {ongoingOrders.length === 0 ? (
                            ...
                            <div className="space-y-4">
                                {ongoingOrders.map((ord) => (
                                    <div
                                        key={ord.id}
                                        className="bg-cream-bg border border-accent/20 hover:border-emerald-700/50 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all"
                                    >
                                        <div
                                            onClick={() =>
                                                setSelectedOrder(ord)
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-earth-text/70 mt-0.5 font-medium">
                                                        Client: {ord.client}{" "}
                                                        &bull;{" "}
                                                        <span className="font-mono text-earth-text/50">
                                                            {ord.id}
                                                        </span>
                                                    </p>
                                                </div>
                                            ...
                                            <div className="text-right">
                                                <span className="text-[9px] font-mono font-bold text-emerald-800 uppercase tracking-widest bg-emerald-600/10 border border-emerald-600/20 px-2.5 py-1 rounded-full">
                                                    {ord.status}
                                                </span>
                                                <span className="text-[10px] font-bold text-earth-text/50 group-hover:text-accent transition-colors mt-1 block">
                                                    Open Details ↗
                                        ...
                                        <div className="space-y-3 pt-3 mt-3 border-t border-accent/15">
                                            <div className="space-y-1.5">
                                                <div className="flex justify-between items-center text-xs font-bold">
                                                    ...
                                                    <span className="text-earth-text font-mono">
                                                        {ord.progress ?? 0}%
                                                    </span>
                                                </div>
                                ...
                                {completeOrders.map((ord) => (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <div className="text-right">
                                                <span className="text-[9px] font-mono font-bold text-earth-text/70 uppercase tracking-widest bg-warm-beige border border-accent/20 px-2.5 py-1 rounded-full">
                                                    {ord.status}
                                                </span>
                                                <span className="text-[10px] font-bold text-earth-text/50 group-hover:text-accent transition-colors mt-1 block">
                                                    Open Details ↗
            ...
            {selectedOrder && (
                <div
                    className="fixed inset-0 bg-earth-text/60 backdrop-blur-md z-9999 flex items-center justify-center p-4 sm:p-6"
                ...
                >
                    ...
                    >
                        <div className="p-5 border-b border-accent/20 flex justify-between items-center bg-warm-beige">
                            <div className="flex items-center gap-3">
                                <span className="text-accent bg-accent/10 px-2.5 py-1 rounded-lg text-xs font-bold font-mono border border-accent/20">
                                    {selectedOrder.id}
                                </span>
