...
The gateway used after Firebase authentication to resolve the application role. Firebase manages identity and sessions; business profiles and role mappings are stored in PostgreSQL and exposed through the backend API. No Firestore collection is used by this frontend.

| Method | Endpoint        | Auth Guard  | Description                                                                                                                                                                                                                                                                     |
| ------ | --------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`  | `/auth/me/role` | Valid Token | Verifies the Firebase bearer token and returns the user's database role (`client` or `tailor`). A missing role returns `404`, which sends the user to frontend onboarding. The frontend chooses `/client/home` or `/tailor/home`; it does not depend on a backend redirect URL. |
...
> [!NOTE]
> Registration first creates the Firebase account, then sends profile data with the Firebase ID token. A tailor registration also creates a shop and can attach a shop image.
...
| `POST` | `/profiles/client`                   | Valid Token | Create the authenticated client's profile and role during registration/onboarding. |
| `POST` | `/profiles/tailor`                   | Valid Token | Create the authenticated tailor profile and role during registration/onboarding.   |
