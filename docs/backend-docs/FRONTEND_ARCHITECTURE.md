...
- User selects their role (`client` or `tailor`) and fills in profile fields.
- Frontend calls `firebaseUser.getIdToken()` to get the Firebase ID Token.
- Depending on the selected role, the frontend POSTs to:
  - `POST /api/v1/profiles/client` (Empty payload `{}`, UID extracted from token)
  - `POST /api/v1/profiles/tailor` (Payload includes `specialty` and optional NIC fields)
