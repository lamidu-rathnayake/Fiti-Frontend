...
- Payload for clients is empty (`{}`); tailors send `specialty` and NIC uploads
- Firebase ID Token sent as Bearer token in `Authorization` header (Backend extracts UID from this)
