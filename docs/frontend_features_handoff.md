...
The backend natively supports a complete role-based onboarding flow using Firebase Auth.
...
*   **Role Routing Mechanism (Industry Standard Frontend-Driven Routing)**: 
    ...
    *   **API Update**: The backend no longer returns a hardcoded `redirect_to` field. It only returns the user's `role` (e.g., `"client"` or `"tailor"`).
...
*   **Client Onboarding**: 
    *   **Feature**: Form to collect phone, city, and address.
    *   **API**: `POST /api/v1/profiles/client`
*   **Tailor/tailor Onboarding**: 
    *   **Feature**: 2-step form to collect personal info, NIC images for verification, and create their first Shop profile.
...
*   **Create Clothing Request**:
    ...
    *   **New Enhancements Ready**: 
        1. **Voice Notes**: Add a microphone button to record audio instructions, upload to Firebase Storage, and pass the `voice_note_url`.
        2. **Inspiration Gallery**: Allow users to upload multiple Pinterest/design screenshots (`design_image_urls`).
        3. **Service Type Toggle**: Let users select `online` or `physical_visit` delivery methods.
